...
// START DEFAULT PLAYER FIX
// Note: removing default vjs player because it cannot play
// correctly. Using jwsplayersup. jwsplayersup has an infinite loading issue
// when play() is blocked by browser policy with no user interaction.
// Adding a script to check if AudioContext is suspended by browser also
// and if yes, stop the loading.
// add_filter( 'addplayers', 'vjsup' ); 	
// add_filter( 'addplayers', 'wavesup' );
add_filter( 'addplayers', 'jwplayersup' );
add_action('vibe_footer','s33d' );
function s33d() {
echo('<script> $(document).ready(   function(){  var acc = new AudioContext();if ( acc.state === "suspended"){jwplayer().once("meta", function() {jwplayer().stop(); }) } })</script>');
}
// END DEFAULT PLAYER FIX
